...
class Logger():
    """
    logs http traffic to STDOUT/file (TAB-delimited)
...
class HTTPProxyClient(protocol.Protocol):
...
class HTTPProxyClientFactory(protocol.ClientFactory):
...
class HTTPProxyServer(protocol.Protocol):
    def connectionMade(self):
        ...
        factory = HTTPProxyClientFactory(self.sq, self.cq, self.log)
...
class HTTPProxyServerFactory(protocol.ServerFactory):
...
def main():
    ...
    factory = HTTPProxyServerFactory(opts.host, opts.port, log)
